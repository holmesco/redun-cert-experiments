...
def bunny_stereo_synthetic(plot: bool = False):
    ...
    points_c1 = _world_to_camera(sampled_points, T_w_c1)
    points_c2 = _world_to_camera(sampled_points, T_w_c2)
...
def test_3d_point_reconstruction_euroc(euroc_data, plot=True):
    ...
    if plot:
        ...
        ax.scatter(
            kpt_3D_0[0, valid_0].cpu(),
            kpt_3D_0[1, valid_0].cpu(),
            kpt_3D_0[2, valid_0].cpu(),
            c="blue",
            s=10,
            label="frame 0",
        )
        ax.scatter(
            kpt_3D_1_in_0[0, valid_1].cpu(),
            kpt_3D_1_in_0[1, valid_1].cpu(),
            kpt_3D_1_in_0[2, valid_1].cpu(),
            c="red",
            s=10,
            label="frame 1 (transformed to frame 0)",
        )
    ...
    kpt_diff_valid = kpt_diff[:, valid_matches]
    print(kpt_diff_valid)
    tolerance = 10e-2  # 10 cm error tolerance
    num_inliers = torch.sum(torch.norm(kpt_diff_valid[:3, :], dim=0) < tolerance).item()
    assert (
        num_inliers > 10
