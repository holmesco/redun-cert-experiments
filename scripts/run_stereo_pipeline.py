...
def plot_pointclouds(kpt_3D_0, kpt_3D_2, inliers, ax=None, title="3D Keypoints", plot_outliers=False):
    if ax is None:
        fig = plt.figure()
...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        config.feature_extractor_config.max_num_keypoints = 1024*2
        ...
        T_init = torch.tensor(
            T_01, dtype=torch.float32, device="cpu"
        )  # initial guess for the relative transform
        ...
        print(f"Translation error: {trans_error:.4f} m, Rotation error: {rot_error:.4f} rad")
        ...
        plot_pointclouds(kpt_3D_0, kpt_3D_1, inliers, ax[0], title="3D Keypoints (No Correction)", plot_outliers=plot_outliers)
        # Plot 3D Matches with correction
        plot_pointclouds(kpt_3D_0, kpt_3D_1_in_0, inliers,ax[1], title="3D Keypoints (Transformed Frame 1 to Frame 0)", plot_outliers=plot_outliers)
        plt.show()